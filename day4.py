...
def parse_passports():
    passports = []
    with open("data/day4.dat") as fdata:
        tmp = {}
        for line in fdata:
            attr = line.split(" ")
            ...
            for a in attr:
                asplit = a.split(":")
                if len(asplit) > 1:
                    tmp[asplit[0]] = asplit[1].replace("\n", "")
        passports.append(tmp)
    return passports


def get_valid_passports():
    valid_attr = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']

    count = 0
    for p in parse_passports():
        attr_count = 0
        for a in valid_attr:
            attr_count += 1 if a in p else 0
        count += 1 if attr_count == len(valid_attr) else 0

    return count


def get_valid_passports2():
    valid_rules = {
        'byr': [1920, 2002],
        'iyr': [2010, 2020],
        'eyr': [2020, 2030],
    ...
    for p in parse_passports():
        attr_count = 0
        for a in valid_rules:
            if a not in p:
                continue

            if isinstance(valid_rules[a], list) and len(valid_rules[a]) == 2:
                # Integer in between
                attr_count += 1 if valid_rules[a][0] <= int(p[a]) <= valid_rules[a][1] else 0
            elif isinstance(valid_rules[a], list) and len(valid_rules[a]) > 2:
                # Value in the list
                attr_count += 1 if p[a] in valid_rules[a] else 0
            elif isinstance(valid_rules[a], dict):
                metric = p[a][-2:]
                if metric not in ['in', 'cm']:
                    continue
                attr_count += 1 if valid_rules[a][metric][0] <= int(p[a][:-2]) <= valid_rules[a][metric][1] else 0
            else:
                attr_count += 1 if re.match(valid_rules[a], p[a]) else 0

        count += 1 if attr_count == len(valid_rules) else 0

    return count

get_valid_passports2()
